...
| Truncated | After populating the list, append a final disabled item: "Showing first {N} issues (list may be incomplete)" in `subtext` color. Displayed only when the `truncated` flag is `True` in `issues_loaded` |
...
- Issue list and notes are fetched up to MAX_PAGES pages (2000 items at `per_page=100`). Projects exceeding this limit will show a truncation warning in the list widget
