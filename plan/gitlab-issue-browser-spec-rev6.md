...
| 6   | 2026-04-10 | Review integration (pascal C-5, euler C-1) |
...
- `[v6] pascal:C-5 + euler:C-1` §9.7: Cache hit path now sets `_current_list_request_id = -1` before returning, invalidating any in-flight list request and preventing stale responses from overwriting cached UI. Additionally, `_on_issues_loaded` and `_on_list_error` now verify `(project, state)` matches `_selected_project` and current filter, as a secondary guard against cross-project stale responses
...
<!-- [v6] pascal:C-5 + euler:C-1: cache hit invalidates request_id; handlers verify project/state -->

`MainWindow` maintains `_current_list_request_id: int` and `_current_detail_request_id: int`, updated each time a new fetch is initiated. When a cache hit serves the UI directly, `_current_list_request_id` is set to `-1` to indicate no in-flight request is expected, preventing any late-arriving stale response from being accepted.
...
def _on_project_clicked(self, project: str):
    ...
    if cached is not None:
        # [v6] pascal:C-5 + euler:C-1: invalidate request_id so no stale response
        # can match after cache hit — no in-flight request is expected
...
def _on_issues_loaded(self, project: str, state: str, request_id: int, truncated: bool, issues: list):
    ...
    if request_id != self._current_list_request_id:
        ...
        return
    # [v6] euler:C-1: secondary guard — verify project/state matches current context
