...
- `ProjectCard` emits a `clicked(str)` signal on left mouse button press, passing `project_path` (full path for GITLAB_PROJECTS entries, `None` for dynamic-only cards)
...
- If clicked card has `project_path = None`, Issue Mode is not entered (no GitLab API target). Click is ignored for issue browsing; only the selection highlight changes
...
```python
# [v3] euler:C-1 + euler:C-2: name resolution map
# Built at __init__ from GITLAB_PROJECTS
_name_to_path = {}
...
3. If not found, create a new dynamic card with `project_path = None` (§5.2)

This ensures pre-populated cards and Discord-updated cards are **always the same card**, never duplicated.
...
def _fetch_all_pages(self, url: str, params: dict) -> list[dict]:
    ...
    while next_url is not None:
        ...
        if page_count >= MAX_PAGES:
            break  # [v3] dijkstra:C-6: stop at limit
...
def _process_list_request(self, project, state, request_id):
    try:
        issues = self._fetch_all_pages(url, params)
        self.issues_loaded.emit(project, state, request_id, issues)
    except _ShutdownInterrupt:
...
- Issue list and notes are fetched up to MAX_PAGES pages (2000 items at `per_page=100`). Projects exceeding this limit will show a truncated list
